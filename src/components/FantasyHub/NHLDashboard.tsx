...
const fetchNHLGames = async (date?: string): Promise<NHLGame[]> => {
  try {
    ...
    // The backend returns { games, count, date, source }
...
const fetchNHLStandings = async (): Promise<NHLStanding[]> => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/nhl/standings`);
    if (!response.ok) throw new Error('Failed to fetch standings');
    const data = await response.json();
    return data.standings && data.standings.length > 0 ? data.standings : mockStandings;
  } catch (error) {
    console.error('Error fetching NHL standings, using mock data:', error);
...
const fetchNHLPlayers = async (limit: number = 30): Promise<NHLPlayer[]> => {
  try {
    ...
    // Real players are nested under json.data.players
    const players = json?.data?.players;
    return players && players.length > 0 ? players : mockPlayers;
  } catch (error) {
...
const NHLDashboard: React.FC = () => {
  ...
  // Filter players by position and type
