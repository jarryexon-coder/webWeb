...
const ParlayArchitectScreen = () => {
  ...
  // Use custom hooks - FIXED: Remove undefined parameter causing issues
  const { 
    data: parlayData, 
    isLoading: parlayLoading, 
    error: parlayError, 
    refetch: refetchParlays 
  } = useParlaySuggestions(selectedSport === 'all' ? 'all' : selectedSport, 4);
  ...
  // Generate parlay from today's games - FIXED: Added proper null checks
  const generateParlayFromGames = useCallback(async (sport: string, numLegs: number) => {
    ...
    try {
      ...
      const newParlay: ParlaySuggestion = {
        ...
        analysis: 'AI-generated parlay based on today\'s matchups.',
  ...
  // Apply filters - FIXED: Removed infinite loop
  useEffect(() => {
...
// Debug Panel Component - FIXED: Removed infinite update loop
const DebugPanel = () => {
  ...
  useEffect(() => {
    ...
    updateStats();
    // Update less frequently to avoid infinite loops
...
// Today's Games Component - FIXED: Added proper error handling
const TodaysGamesPanel = () => {
  if (!todaysGames) return null;
      ...
      <Accordion expanded={showTodaysGames} onChange={() => setShowTodaysGames(!showTodaysGames)}>
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <Box display="flex" alignItems="center" gap={2}>
            ...
            <Chip 
              label={`${todaysGames.length} games`} 
              size="small" 
        ...
        <AccordionDetails>
          {oddsLoading ? (
            ...
            <Alert severity="warning">
              ...
              The odds API is currently unavailable.
            </Alert>
            ...
            <Grid container spacing={2}>
              {todaysGames.slice(0, 6).map((game: Game) => (
                  ...
                  {parlay.confidence !== undefined && (
                    <Box sx={{ mb: 2, mt: 1 }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 0.5 }}>
                        <Typography variant="caption" color="text.secondary">
                          AI Confidence: {parlay.confidence_level?.replace('-', ' ') || 'medium'}
                        </Typography>
